...
void SVGPath::createPath(std::string path) {
        ...
        if (valid_commands.find(commands[i]) != std::string::npos) {
            ...
            // Cubic bezier curve, absolute coordinates
                ...
                for (int j = 0; j != float_data[i].size(); j += 6) {
                    ...
                    for (int k = 1; k != cubic_bezier_count + 1; ++k) {
                        sf::Vector2f pos(cubicBezier(p1, p2, p3, p4, 1.f / cubic_bezier_count * k));
                        lineTo(pos.x, pos.y);
                    }
            ...
            // cubic bezier curve, relative coordinates
                ...
                for (int j = 0; j != float_data[i].size(); j += 6) {
                    ...
                    sf::Vector2f p1(x, y);
                    sf::Vector2f p2 = p1 + sf::Vector2f(float_data[i][j], float_data[i][j + 1]);
                    sf::Vector2f p3 = p1 + sf::Vector2f(float_data[i][j + 2], float_data[i][j + 3]);
                    sf::Vector2f p4 = p1 + sf::Vector2f(float_data[i][j + 4], float_data[i][j + 5]);
            ...
            /// DOES NOT WORK
                ...
                for (int j = 0; j != float_data[i].size(); j += 4) { 
                    ...
                    for (int k = 1; k != quadratic_bezier_count + 1; ++k) {
                        sf::Vector2f pos(quadraticBezier(p1, p2, p3, 1.f / quadratic_bezier_count * k));
                        lineTo(pos.x, pos.y);
                    }
    ...
    // now actually create the geom
    createGeom();
}

void SVGPath::lineTo(float x, float y) {
    points.push_back(sf::Vector2f(x, y));
}

bool is_duplicate();

void SVGPath::deleteDuplicates() {
